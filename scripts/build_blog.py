...
<main>
  <article>
    <h1>{h1}</h1>
    <span class="updated">公開日：{pub_j} ｜ 更新日：{mod_j} ｜ ミニロト分析ナビ運営事務局</span>
{body}
  </article>
</main>
<footer class="site">
...
A('basics.html',              '2026-07-20', 'ミニロトの基本ルールと確率の仕組み', '1〜31から5個を選ぶミニロトのルール、等級の条件、組合せ169,911通りと各等の理論確率を初心者向けに整理しました。', '基本')
A('combination-formula.html', '2026-07-21', '組合せの数の公式でミニロトを分解する', 'なぜ169,911通りなのか。掛け算と割り算だけで導ける組合せの公式を、各等の確率計算と一緒にやさしく解説。', '基本')
A('loto-types.html',          '2026-07-22', 'ミニロト・ロト6・ロト7・ナンバーズ — 種類を比較', '日本の数字選択式宝くじの違い（価格・抽選日・1等確率・賞金規模）を表で整理。自分に合う種類の選び方。', '基本')
A('win-reality.html',         '2026-07-23', 'ミニロト当選確率の現実 — 数字は正直です', '1等1/169,911を日常の感覚に置き換え、払戻率50%未満という期待値の現実まで。運営が正直に整理しました。', '基本')
A('faq.html',                 '2026-08-01', 'ミニロトよくある質問（FAQ）', 'クイックピックと自選はどちらがいい？連番は出る？複数口は1回で買うべき？など、統計サイト運営の立場から率直に回答。', '基本')

A('top10-numbers.html',       '2026-07-28', '出現回数が多い数字 TOP10【第1回〜第1397回】', '当サイトの全データ6,985個分を集計。出現トップの11は265回。誤差の範囲と読み方まで実測で示します。', '実録')
A('absent-numbers.html',      '2026-07-29', 'いま出ていない数字の分析【第1397回時点】', '最新回時点で最長未出現は15の18回分。18回連続で出ない確率は約4%で意外と起きる話、を計算で解説。', '実録')
A('consecutive-numbers.html', '2026-07-30', '連番はどのくらい出ている？全1,397回で調査', '直近でも25・26・27の3連番が出ています。連番入りの回は52.5%。「連番は珍しい」はデータでどうなるか。', '実録')
A('winners-analysis.html',    '2026-07-26', '当選の現実 — 1回の抽選で何人が当たる？', '最新回は1等21口・4等51,536口。約180万円の回から満額4,000万円の回まで。データで見る賞金のリアル。', '実録')
A('statistics-reading.html',  '2026-07-22', '出現回数・未出現期間の正しい読み方', '統計画面に出る「出現回数」「未出現期間」「長期間出ていない数字」の意味と、勘違いしやすいポイントを解説。', '実録')
A('independence.html',        '2026-07-24', '過去データは未来を予測できるか — 独立抽選の考え方', '「連続して出た数字は次も出る？」など、よくある誤解の背景にあるギャンブラーの誤謬と、統計の健全な使い方。', '実録')
A('prize-structure.html',     '2026-07-29', '賞金額が毎回違う理由 — キャリーオーバーと分配の仕組み', '同じ1等なのに回によって賞金が違うのはなぜ？売上からの払戻・当選口数による分割とキャリーオーバーを解説。', '実録')

A('quickpick-vs-jisen.html',  '2026-07-25', 'クイックピックと自選、結局どちらがいいのか', '確率はまったく同じ。では何で選ぶべきか。記録・検証できる自選派向けの使い方まで運営視点で整理。', '実践')
A('avoid-patterns.html',      '2026-07-27', 'できれば避けたい組合せパターン（分割リスクの話）', '確率は同じでも「当たった時に損」になりやすい人気パターンがある。誕生日・ゾロ目・1-2-3-4-5の罠。', '実践')
A('backtest.html',            '2026-07-26', 'バックテストで選び方を検証する方法', '「この選び方、過去ならどうだった？」を確かめる方法。ランダム基準線との比較の読み取り方と注意点。', '実践')
A('coverage.html',            '2026-07-27', 'カバレッジ設計入門 — 少ない枚数で広くカバーする考え方', '複数口を組み合わせて広範囲を押さえるウィーリングの基礎と、枚数とのトレードオフ。ツールの使い方つき。', '実践')
A('statistics-guide.html',    '2026-08-01', 'ミニロト統計の見方・完全ガイド（当サイトの使い方）', '10個のメニューをどう使い分けるか。初めての方向けの3ステップから、データを読む実践まで運営が案内。', '実践')
A('marksheet-guide.html',     '2026-08-02', 'マークシートの書き方と購入の流れ', 'A〜Eブロックの見方、マークのコツ、クイックピック欄の使い方。当サイトの印刷ツールで事前に整理する方法も。', '実践')
A('responsible-play.html',    '2026-07-31', '予算管理と責任ある遊び方 — 健全に楽しむために', '宝くじは「余裕資金の娯楽」が原則。購入前に決めたい予算ルール、注意したいサイン、20歳以上のルール。', '実践')
A('cashing.html',             '2026-08-01', '当選したらどうする？換金の流れと注意点', '当選択確認から換金場所・本人確認・支払期限まで。うっかり損をしないための基本の流れを整理。', '実践')
...
'''
    shell('index.html', 'コラム一覧', 'ミニロトのルール・確率・統計の読み方・検証・健全な楽しみ方を解説するコラム一覧。',
          'コラム一覧', 'コラム・読みもの', '2026-07-20', index_body)
